...
export default class GameController extends cc.Component {
  ...
  onLoad() {
    ...
    [this.winPopup, this.losePopup].forEach(popup => {
      ...
      if (restartBtn) {
        restartBtn.getComponent(cc.Button)!.node.on(
          "click",
          this.onRestartDefault,
          this
        );
      }
      ...
      if (customBtn) {
        customBtn.getComponent(cc.Button)!.node.on(
          "click",
          this.onShowCustom,
          this
        );
  ...
  private _restartGame(rows: number, cols: number, moves: number, target: number) {
    // спрячем все попапы
    this.popupContainer.active = false;
    this.winPopup.active       = false;
    this.losePopup.active      = false;
  ...
  private renderGrid() {
    ...
    for (const rowArr of this.model.board.grid) {
      for (const tile of rowArr) {
        ...
        node.getComponent("TileView")!.init(
          tile,
